...
func (c *CooldownTracker) MarkHunted(instanceID string, episodeID int) {
	c.mu.Lock()
	defer c.mu.Unlock()
	key := fmt.Sprintf("%s:%d", instanceID, episodeID)
	c.lastHunt[key] = time.Now()
}
